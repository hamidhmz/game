										...
										<div class="center">
											...
											<span class="btn btn-app btn-lg btn-danger no-hover">
												<span class="line-height-1 bigger-170">
													<?php
														$j = 0;
														$sql = "select * from chat where status=0";
												...
												<br />
												<span class="line-height-1 smaller-90">تعداد تیکت های دیده نشده</span>
											</span>
											...
											<span class="btn btn-app btn-lg btn-purple no-hover">
												...
												<br />
												<span class="line-height-1 smaller-90">تعداد کل تیکت ها</span>
											</span>
										...
										<div class="widget-box widget-color-pink" id="recent-box">
											<div class="widget-header">
												<h4 class="widget-title lighter smaller">
													<i class="ace-icon fa fa-user-o blue"></i>کل جزییات تیکت ها
												</h4>
											</div>
														...
														<table id="dynamic-table" class="table table-striped table-bordered table-hover">
															<thead>
																<tr>
																	...
																	<th>نام</th>
																	<th>تعداد پیام های دیده نشده </th>
																	<th>تعداد پیام ها</th>
																	<th >وضعیت</th>
																	<th></th>
																</tr>
															...
															<tbody>
															<?php
																$sql="SELECT * FROM `tmpuser` ORDER BY `status` ASC";
																$result=$connect->query($sql);
																while($fetch=$result->fetch(PDO::FETCH_ASSOC))
																{
															...
															?>
																<tr>
																	...
																	<td><?php
																		$sqlp1="SELECT COUNT(id) as tem FROM `chat` WHERE id_user=:id AND status=0";
																		$resultp1=$connect->prepare($sqlp1);
																		$resultp1->bindvalue(":id",$fetch['id']);
																		$resultp1->execute();
																		$fetchp1=$resultp1->fetch(PDO::FETCH_ASSOC);
																		echo $fetchp1['tem'];
																	?></td>
																	<td><?php
																		$sqlp2="SELECT COUNT(id) as tem FROM `chat` WHERE id_user=:id ";
																		...
																		echo $fetchp2['tem'];
																	?></td>
																	<td class="h-tex"><?php if($fetch['status']==0) {echo "<span class='label label-success'>پیام جدید</span>";}else{echo "دیده شده";} ?></td>
																	<td>
																		<div class=" action-buttons">
																			<a class="blue open-event" href=
																			<?= "'chats/?id=" . $fetch['id'] . "'";?> data-toggle="modal" title="مشاهده جزییات">
