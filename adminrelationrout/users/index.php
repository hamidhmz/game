						...
						<div class="page-header">
							<h1>
								...
								<small>
									...
									نمایی کلی از جزییات کاربران قدیمی و جدید
								</small>
							</h1>
						</div><!-- /.page-header -->
									...
									<div class="col-sm-12">
										<div class="center">
											...
											<span class="btn btn-app btn-lg btn-warning no-hover">
												...
												<br />
												<span class="line-height-1 smaller-90">تعداد کل اعضا</span>
											</span>

											<span class="btn btn-app btn-lg btn-purple no-hover">
												<span class="line-height-1 bigger-170">
													<?php
														$sql="SELECT COUNT(id) as tid FROM tmpuser where active='1'";
													    $result=$connect->query($sql);
													    $fetch=$result->fetch(PDO::FETCH_ASSOC);
													    echo $english_format_number = number_format($fetch['tid']);
												    ?>
												...
												<br />
												<span class="line-height-1 smaller-90">تعداد اعضای فعال</span>
											</span>
										...
										<div class="widget-box widget-color-pink" id="recent-box">
														...
														<table id="dynamic-table" class="table table-striped table-bordered table-hover">
															<thead>
																<tr>
																	...
																	<th>نام واقعی</th>
																	<th>شماره تلفن</th>
																	<th>مقدار پول واریز شده توسط این کاربر</th>
																	<th>مقدار پول دریافت شده توسط این کاربر</th>
																	<th class="hidden-480">وضعیت</th>
																	<th>آدرس ایمیل</th>
																	<th>درآمد سایت از این کاربر</th>
																	<th></th>
																</tr>
															</thead>
															...
															?>
																<tr>
																	...
																	<?php
																		$add = 0;
																		$who = $fetch['id'];
																		$sql1="select * from tmpaddmoney where id_user=:user ";
																    $result1=$connect->prepare($sql1);
				                            $result1->bindvalue(":user",$who);
				                            $result1->execute();
				                            ...
				                            echo $english_format_number = number_format($add);
																	...
																	<?php
																		$get = 0;
																		$who = $fetch['id'];
																		$sql1="select * from tmpgetmoney where id_user=:user ";
																    $result1=$connect->prepare($sql1);
				                            $result1->bindvalue(":user",$who);
				                            $result1->execute();
				                            while($fetch1=$result1->fetch(PDO::FETCH_ASSOC))
				                            ...
				                            echo $english_format_number = number_format($get);
																	...
																	<?php
																		if ($fetch['active'] == '1') {
																			echo "<span class='label label-sm label-warning'>فعال</span>";
																		}
																		else
																		{
																			echo "<span class='label label-sm label-warning'>غیر فعال</span>";
																		}
																	?>
																	</td>
																	<td><?= $fetch['email'] ?></td>
																	...
																	<td>
																		...
																		<?php if ($fetch['active'] == '1') {
																			?>
																				<a class="red open-event" href="delete.php?inactive=<?= $fetch['id'] ?>" title="غیر فعال کردن کاربر">
																			...
																			<?php
																				...
																				<a class="green open-event" href="delete.php?active=<?= $fetch['id'] ?>" title="فعال کردن کاربر">
