...
/* @var $model common\models\LigsGames */

$user_id       = Yii::$app->user->id;
$type_title    = $model->lig->type->title;
$total_amount  = number_format($model->lig->total_amount) . ' تومان';

$player1id     = $model->player_1;
$player1name   = $model->player1->fullname;
$player1dice   = Url::to(['/themes/sixpairs/images/black.png']);
$player1boards = $model->getLigsGamesBoards()->where(['status_id' => 2, 'winner_id' => $player1id])->count();

$player2id     = $model->player_2;
$player2name   = $model->player2->fullname;
$player2dice   = Url::to(['/themes/sixpairs/images/white.png']);
$player2boards = $model->getLigsGamesBoards()->where(['status_id' => 2, 'winner_id' => $player2id])->count();
...
$this->registerJsFile('@web/themes/sixpairs/js/ligs_play.js?ver=' . $ver, $depends);
?>