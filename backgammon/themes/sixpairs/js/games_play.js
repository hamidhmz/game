...
$(function () {
    var socket = io(server_url, {query: 'p=games_play&i=1&g=1'});
    socket.on('connect', function () {
        $('#connecting').hide();
        socket.emit('games_play', username, password, game_id);
    });
    ...
    socket.on('finish game', function (winner) {
        if (winner !== type) {
            $('#loserName').text(myName);
            $('#winnerName').text(opponentName);

        }
        else {
            $('#winnerName').text(myName);
            $('#loserName').text(opponentName);
        }
...
function redrawCheckers(socket, data) {
    ...
    for (var i = 0; i < checkersCount; i++) {
        ...
        if (type === 0) {
            ...
            element = generateSmallBrownChecker().css('top', (i * 6) + '%').css('left', '4px');
        }
    ...
    for (var i = 0; i < checkersCount; i++) {
        ...
        if (type === 0) {
            ...
            element = generateSmallWhiteChecker().css('bottom', (i * 6) + '%').css('left', '4px');
        }
    ...
    if (availableMoves.length && currentPlayer === type) {
        ...
        $(availableCheckers).dblclick(function () {
            ...
            if (target) {
                ...
                socket.emit('do move', {gid: game_id, source: source, target: target});
                $('.undo').show();
            }
        }).draggable({
            revert: "invalid",
